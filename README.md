...
Локально на n8n 2.39.8: валидная заявка → сообщение в Telegram + строка в таблице `leads` + ответ `200`;
невалидная → ответ `400` со списком ошибок, ничего не сохраняется. Кириллица в именах и сообщениях сохраняется корректно.

## Ограничения

- Телефон нормализуется только по цифрам: российский `8 (912) ...` сохранится как `+8912...`, а не `+7912...` (конвертацию 8→7 ещё не добавлял).
